...
uint64_t
mc_hash_key(const char * str, int sz) {
...
int
mc_encrypt(const char *in, int sz, char *out, uint64_t key, time_t ti) {
...
int
mc_decrypt(const char *in, int sz, char *out, uint64_t key, time_t ti) {
   uint32_t pt, check;
   uint64_t h;
   struct rc4_sbox rs;
   sz -= 8;
